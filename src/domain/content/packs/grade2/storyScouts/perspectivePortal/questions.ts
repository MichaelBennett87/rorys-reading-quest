...
function buildFiveQuestionLesson(
  ...
  return [
    ...
    ht({
      ...
      prompt: `Select the sentence that states ${b.characterName}'s first view of what should happen.`,
      explanation: `This sentence states ${b.characterName}'s first view of what should happen in the shared situation.`,
      evidenceReference: 'perspective-sentence-b',
      evidenceReferenceIds: sentenceIds(blueprint, ...b.supportingSentenceNumbers.slice(0, 1)),
...
function buildCheckpointQuestions(
  ...
  return [
    ...
    ms({
      ...
      lessonIdentifier: lessonId,
      questionIdentifier: perspectivePortalQuestionId(lessonKey, 'ms-1'),
      prompt: `Choose two details: one that states ${a.characterName}'s view and one that shows ${a.characterName} acting on it.`,
      explanation: `One detail states this perspective, and one shows ${a.characterName} acting on it: ${a.perspectiveStatement}`,
    ...
    ep({
      ...
      partBPrompt: `Which detail shows ${a.characterName} acting on that perspective?`,
      partBChoices: [
        { id: 'choice-a', text: blueprint.sentences[(a.supportingSentenceNumbers[0] ?? 2) - 1] },
        { id: 'choice-b', text: blueprint.sentences[(b.supportingSentenceNumbers[0] ?? 3) - 1] },
        { id: 'choice-c', text: blueprint.sentences[(a.supportingSentenceNumbers[1] ?? 4) - 1] },
