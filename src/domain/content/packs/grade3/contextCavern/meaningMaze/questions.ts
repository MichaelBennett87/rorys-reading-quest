...
const table = (context: QuestionContext, index: number, prompt: string, rowPrompts: string[], meanings: string[], correctIndexes: number[], explanation: string, refs: number[], vocabulary: string[]) => {
  const questionId = context.questionIds[index]!
  const sharedChoices = choices(`${questionId}-meaning`, [...meanings, 'not supported by this source'])
...
const referenceQuestions: ReadingQuestion[] = [
  ...
  multi(contexts.reference, 2, 'Choose two clues that support the meaning of route: a reference clue and a source clue.', ['The glossary says route is a path used to travel.', 'The map line runs from the garden to the pond.', 'The dictionary says observe can mean honor a holiday.', 'The final pond path is shaded.'], [0, 1], 'The glossary and mapped path work together to establish the travel-path meaning.', [4, 5, 10], ['route'], ['reference-plus-context']),
...
const figurativeQuestions: ReadingQuestion[] = [
  mc(contexts.figurative, 0, 'What does a blanket of fog mean in the poem?', ['thick fog covered the view', 'a cloth blanket lay on the trail', 'snow covered every stone', 'the group slept beside the path'], 0, 'The path is hidden until warm light thins the gray fog.', [1, 2, 3, 4], ['a blanket of fog'], ['figurative-meaning']),
  mc(contexts.figurative, 1, 'What does the idea took root mean?', ['the group accepted and developed the idea', 'the idea became a plant', 'roots blocked the trail', 'Mira forgot the idea'], 0, 'Everyone adds a useful step, and the plan begins to grow.', [5, 6, 7], ['the idea took root'], ['figurative-meaning']),
  mc(contexts.figurative, 2, 'A new project note says, “The plan hit a snag when the only wheel cracked.” Which strategy best confirms the phrase means the plan met a problem?', ['Combine the cracked-wheel result with the project context.', 'Picture a paper plan caught on a sharp branch.', 'Name a figurative device without checking the event.', 'Use only the familiar meaning of the word hit.'], 0, 'The cracked wheel interrupts the project, so event context confirms a problem and rejects the literal branch reading.', [8, 9], ['the plan hit a snag'], ['transfer', 'strategy-selection', 'literal-reading-rejection']),
  multi(contexts.figurative, 3, 'Choose the two details in the line immediately after time slipped away that show time passed before the work was finished. Select two.', ['The sun moved low.', 'The final loop was still unfinished.', 'The group opened its map in the morning.', 'The bridge board had washed away.'], [0, 1], 'The low sun and unfinished work show that the available day passed.', [10, 11, 12], ['time slipped away'], ['figurative-context']),
  hot(contexts.figurative, 4, 'Select the next line after the idea took root that confirms the idea was accepted and developed by the group.', [5, 6, 7, 9], 2, 'The plan begins to grow after everyone adds a step, confirming the figurative phrase.', ['the idea took root'], ['context-confirmation']),
  table(contexts.figurative, 5, 'Match each phrase to its intended meaning.', ['a blanket of fog', 'the idea took root', 'the plan hit a snag', 'time slipped away'], ['thick fog covered the view', 'the group developed an idea', 'the plan met a problem', 'time passed quickly'], [0, 1, 2, 3], 'Each meaning fits the complete stanza and rejects an impossible literal reading.', [2, 6, 8, 10], ['blanket', 'root', 'snag', 'slipped']),
  twoPart(contexts.figurative, 6, 'Interpret the phrase and confirm it with the poem.', 'What does a blanket of fog mean?', ['thick fog covered the area', 'someone dropped bedding', 'snow filled the path', 'night arrived suddenly'], 0, 'Which line best confirms that meaning?', ['We waited until warm light thinned the gray,', 'Mira shared a careful thought,', 'Everyone added a useful step,', 'We packed our notes and promised to finish together tomorrow.'], 0, 'Light thinning the gray and revealing trail marks confirms that fog covered the view.', [1, 2, 3, 4], ['a blanket of fog']),
]
