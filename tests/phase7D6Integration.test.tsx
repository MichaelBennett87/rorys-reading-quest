...
const NOW = '2026-08-30T20:30:00.000Z'
const SKILL_ID = 'g3-context-cavern-vocabulary'
const UNIT_ID = 'g3-cc-unit-3'
const VERSION = 'g3-cc-meaning-maze-r0.1.0'
const candidates = getLessonCandidates()
const unit2Checkpoint = candidates.find((candidate) => candidate.skillId === SKILL_ID && candidate.unitId === 'g3-cc-unit-2' && candidate.eligiblePurposes.includes('progression'))!
...
function stateReadyForMeaningMaze() {
  ...
  for (const [skillId, difficulty] of [
    ...
    ['g3-information-detectives-reading', 5], ['g3-across-genres-reading', 4],
...
function completedAttempt(candidate: LessonActivityCandidate, completionId: string): CompletedLessonAttempt {
  return {
    attemptId: completionId, completionId, lessonId: candidate.lessonId, lessonRole: 'CHECKPOINT',
    activityId: candidate.activityId, skillId: candidate.skillId, difficulty: candidate.difficulty,
    questionResults: Array.from({ length: 7 }, (_, index) => ({
      questionId: `${completionId}-question-${index + 1}`, isCorrect: true, isFirstAttemptCorrect: true,
      submittedAnswer: 'correct', correctAnswer: 'correct', explanation: 'Fixture explanation.', evidenceReference: [],
    })),
    accuracy: 100, assistanceCount: 0,
    assistanceSummary: { totalUniqueEvents: 0, targetsHelped: 0, maximumAssistanceLevel: 0, visualHintUsed: false, spokenChunkHelpUsed: false, spokenWordHelpUsed: false, sentenceReadAloudUsed: false },
    assistanceEvents: [], completedAt: NOW, progressionDecisionState: 'VERIFY_MASTERY', reasonCodes: ['independent_evidence'], nextReviewDate: null,
  }
}

describe('Phase 7D6 integration and reconciled one-button journey', () => {
  test('registers only Meaning Maze as the final Grade 3 pack and derives complete ELA.3.V.1.3 coverage', () => {
    ...
    expect(snapshot.rows.find((entry) => entry.benchmarkReference === 'ELA.3.V.1.3')).toMatchObject({
      ...
      contributingPackIds: ['g3-context-cavern-meaning-maze'],
      coveredPatterns: ['context-clues', 'figurative-language', 'word-relationships', 'reference-materials', 'background-knowledge', 'multiple-meaning-words', 'unknown-words', 'unknown-phrases'],
    })
  ...
  test('retires stale Unit 3 no-content, preserves local records, and resumes one authoritative session', () => {
    const state = stateReadyForMeaningMaze()
    state.plannedNextQuest = { status: 'content_needed', purpose: 'progression', skillId: SKILL_ID, difficulty: 3, reason: 'Old Meaning Maze boundary.' }
    state.totalXp = 1960
    state.totalStars = 64
    state.reviewQueue = [{ skillId: SKILL_ID, difficulty: 2, reviewStep: 1, dueAt: NOW }]
    ...
    expect(first).toMatchObject({ status: 'start', lesson: { skillId: SKILL_ID, unitId: UNIT_ID, difficulty: 3, contentVersion: VERSION } })
    expect(repeated).toMatchObject({ status: 'resume', lesson: { skillId: SKILL_ID, unitId: UNIT_ID, difficulty: 3 } })
    ...
    expect(journey.result.current.progress).toMatchObject({
      totalXp: 1960, totalStars: 64,
      reviewQueue: [{ skillId: SKILL_ID, difficulty: 2, reviewStep: 1, dueAt: NOW }],
      skillProgress: { [SKILL_ID]: { currentDifficulty: 3, lastMasteredDifficulty: 2 } },
    })
  ...
  test('rejects a completed stale Unit 2 session before launching Unit 3', () => {
    const state = stateReadyForMeaningMaze()
    const unit2Lesson = getLessonById(unit2Checkpoint.lessonId).lesson!
    const staleSession = createActiveLessonSession(unit2Lesson, 'completed-unit-2-session', NOW)
    state.activeLessonSession = staleSession
    state.completedAttempts = [completedAttempt(unit2Checkpoint, staleSession.sessionId)]
    ...
    expect(launch).toMatchObject({ status: 'start', lesson: { unitId: UNIT_ID, difficulty: 3 } })
  ...
  test('keeps Home at two controls, world cards display-only, and launches without a selector', async () => {
    window.localStorage.setItem(QUEST_PROGRESS_STORAGE_KEY, JSON.stringify(stateReadyForMeaningMaze()))
    render(<App />)
    ...
    expect(await screen.findByText(/Meaning Maze Checkpoint:/)).toBeTruthy()
    expect(screen.queryByText(/choose a world|choose a unit|choose a word|choose a phrase|choose a lesson|another lesson is already open/i)).toBeNull()
  })

  test('keeps guide, references, senses, answers, and planner diagnostics out of schema-v1 persistence', () => {
    ...
    for (const forbidden of ['meaningMazeGuides', 'referenceEntries', 'alternateMeanings', 'literalReading', 'intendedMeaning', 'correctAnswers', 'selectionMode']) expect(serialized).not.toContain(forbidden)
    expect(containsExactOwnProperty(JSON.parse(serialized), 'reasonCode')).toBe(false)
  })

  test('keeps parent and print copy honest while excluding guide and answer metadata', () => {
    ...
    expect(dashboardSource).toContain("summary.benchmarkReference === 'ELA.3.V.1.3'")
    expect(dashboardSource).toContain('Meaning-strategy curriculum coverage: IMPLEMENTED / DRAFT')
    expect(dashboardSource).toContain('Chapter completion does not establish unrestricted vocabulary mastery')
    expect(printSource).toContain("skill.benchmarkReferences.includes('ELA.3.V.1.3')")
    expect(printSource).toContain('Meaning-strategy curriculum coverage: IMPLEMENTED / DRAFT')
    for (const forbidden of ['referenceEntries', 'alternateMeanings', 'literalReading', 'Correct answer:']) expect(printSource).not.toContain(forbidden)
  })
})
