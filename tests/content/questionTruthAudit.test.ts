...
describe('active question truth inventory', () => {
  test('derives every active question exactly once from the production registry', () => {
    ...
    expect(inventory.records).toHaveLength(1122)
    expect(new Set(inventory.records.map((record) => record.questionId)).size).toBe(1122)
  ...
  test('builds a blind projection without keys, explanations, evidence, or guide answers', () => {
    ...
    expect(projection).toHaveLength(1122)
  ...
  test('keeps one current fingerprinted PASS ledger record for every active question', () => {
    ...
    expect(Object.keys(ledgerModules)).toHaveLength(28)
    expect(ledgerRecords).toHaveLength(1122)
    expect(new Set(ledgerRecords.map((record) => record.questionId)).size).toBe(1122)
