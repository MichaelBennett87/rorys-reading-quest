...
describe('Grade 2 Compare Castle Wordplay Watchtower pack', () => {
  test('registers the active Compare Castle pack and updates registry totals', () => {
    ...
    expect(activePackIds.at(-8)).toBe(WORDPLAY_PACK_ID)
    expect(activePackIds.at(-7)).toBe(RETELL_PACK_ID)
    expect(activePackIds.at(-6)).toBe('g2-compare-castle-compare-keep')
    expect(activePackIds.at(-5)).toBe('g3-word-forge-root-reactor')
    expect(activePackIds.at(-4)).toBe('g3-word-forge-suffix-shifter')
    expect(activePackIds.at(-3)).toBe('g3-word-forge-multisyllable-mountain')
    expect(activePackIds.at(-2)).toBe('g3-word-forge-fluency-flight')
    expect(activePackIds.at(-1)).toBe('g3-story-scouts-character-arc-camp')
