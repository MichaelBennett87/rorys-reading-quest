...
describe('Grade 2 Compare Castle Wordplay Watchtower pack', () => {
  test('registers the active Compare Castle pack and updates registry totals', () => {
    ...
    expect(activePackIds.at(-20)).toBe(WORDPLAY_PACK_ID)
    expect(activePackIds.at(-19)).toBe(RETELL_PACK_ID)
    expect(activePackIds.at(-18)).toBe('g2-compare-castle-compare-keep')
    expect(activePackIds.at(-17)).toBe('g3-word-forge-root-reactor')
    expect(activePackIds.at(-16)).toBe('g3-word-forge-suffix-shifter')
    expect(activePackIds.at(-15)).toBe('g3-word-forge-multisyllable-mountain')
    expect(activePackIds.at(-14)).toBe('g3-word-forge-fluency-flight')
    expect(activePackIds.at(-13)).toBe('g3-story-scouts-character-arc-camp')
    expect(activePackIds.at(-12)).toBe('g3-story-scouts-theme-development-trail')
    expect(activePackIds.at(-11)).toBe('g3-story-scouts-perspective-portal')
    expect(activePackIds.at(-10)).toBe('g3-poetry-planet-poem-form-observatory')
    expect(activePackIds.at(-9)).toBe('g3-information-detectives-structure-station')
    expect(activePackIds.at(-8)).toBe('g3-information-detectives-central-idea-engine')
    expect(activePackIds.at(-7)).toBe('g3-information-detectives-purpose-development-path')
    expect(activePackIds.at(-6)).toBe('g3-information-detectives-claim-evidence-court')
    expect(activePackIds.at(-5)).toBe('g3-compare-castle-figurative-fortress')
    expect(activePackIds.at(-4)).toBe('g3-compare-castle-summary-stronghold')
    expect(activePackIds.at(-3)).toBe('g3-compare-castle-author-lens-tower')
    expect(activePackIds.at(-2)).toBe('g3-context-cavern-academic-word-workshop')
    expect(activePackIds.at(-1)).toBe('g3-context-cavern-root-meaning-vault')
