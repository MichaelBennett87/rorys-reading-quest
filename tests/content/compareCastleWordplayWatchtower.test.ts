...
describe('Grade 2 Compare Castle Wordplay Watchtower pack', () => {
  test('registers the active Compare Castle pack and updates registry totals', () => {
    ...
    expect(activePackIds.at(-4)).toBe(WORDPLAY_PACK_ID)
    expect(activePackIds.at(-3)).toBe(RETELL_PACK_ID)
    expect(activePackIds.at(-2)).toBe('g2-compare-castle-compare-keep')
    expect(activePackIds.at(-1)).toBe('g3-word-forge-root-reactor')
