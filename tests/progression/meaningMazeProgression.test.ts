...
function result(candidate: LessonActivityCandidate, accuracy: number, assisted = false): LessonResult {
  ...
  return {
    ...
    questionResults: Array.from({ length: totalQuestions }, (_, index) => ({
      ...
      submittedAnswer: '', correctAnswer: '', explanation: '', evidenceReference: [],
    })),
...
function questStateAtDifficulty(difficulty: 2 | 3) {
...
describe('Meaning Maze progression', () => {
  test('stays locked before Unit 2 completion and opens only Unit 3 at difficulty 3', () => {
    expect(planUnitQuest({ selectedUnitId: UNIT_ID, progress: questStateAtDifficulty(2), availableLessons: allCandidates })).toMatchObject({ status: 'locked' })
    expect(planUnitQuest({ selectedUnitId: UNIT_ID, progress: questStateAtDifficulty(3), availableLessons: allCandidates })).toMatchObject({
      status: 'available', lesson: { skillId: SKILL_ID, unitId: UNIT_ID, difficulty: 3, contentVersion: VERSION },
    })
  })

  test('requires two distinct independent checkpoints and completes the chapter at difficulty 4', () => {
    const initial = createInitialSkillProgress(SKILL_ID, 3, 2)
    const first = apply(initial, checkpoints[0]!, 100)
    expect(first.status).toBe('applied')
    if (first.status !== 'applied') return
    expect(first.decision.decisionState).toBe('VERIFY_MASTERY')
    expect(first.progress.currentDifficulty).toBe(3)
  ...
  test('keeps partial, guidance, remediation, and rebuilding inside Unit 3', () => {
    const initial = createInitialSkillProgress(SKILL_ID, 3, 2)
  ...
  test('does not turn assisted work into independent mastery evidence', () => {
    const assisted = apply(createInitialSkillProgress(SKILL_ID, 3, 2), checkpoints[0]!, 100, true)
    ...
    expect(assisted.progress.currentDifficulty).toBe(3)
  ...
  test('preserves safe recycling and grade, unit, and version review isolation', () => {
    ...
    const diagnostic = selectNextLessonWithDiagnostics({
      ...
      recentActivityUsage: checkpoints.map((candidate, index) => ({
        ...
        difficulty: candidate.difficulty, passageQuestionKeys: [...candidate.passageQuestionKeys],
        contentVersion: candidate.contentVersion, completedAt: `2026-08-30T19:0${index}:00.000Z`,
      })),
    })
    ...
    const unit1 = buildReviewQueueIdentity({ skillId: SKILL_ID, difficulty: 1, unitId: 'g3-cc-unit-1', contentVersion: 'g3-cc-academic-word-r0.1.0' })
    const unit2 = buildReviewQueueIdentity({ skillId: SKILL_ID, difficulty: 2, unitId: 'g3-cc-unit-2', contentVersion: 'g3-cc-root-meaning-r0.1.0' })
    const unit3 = buildReviewQueueIdentity({ skillId: SKILL_ID, difficulty: 3, unitId: UNIT_ID, contentVersion: VERSION })
    const grade2 = buildReviewQueueIdentity({ skillId: 'g2-context-cavern-vocabulary', difficulty: 3, unitId: 'cc-unit-3', contentVersion: 'g2-cc-meaning-clue-r0.1.0' })
    expect(sameReviewQueueIdentity(unit1, unit3)).toBe(false)
    expect(sameReviewQueueIdentity(unit2, unit3)).toBe(false)
    expect(sameReviewQueueIdentity(grade2, unit3)).toBe(false)
  })
})
