...
describe('all-active-question grading truth contract', () => {
  test('accepts every canonical response and rejects every generated adversarial response', () => {
    ...
    expect(questions).toHaveLength(1409)
    expect(new Set(questions.map((question) => question.questionId)).size).toBe(1409)
    expect(reports.reduce((sum, report) => sum + report.canonicalSubmissionCount, 0)).toBe(1409)
