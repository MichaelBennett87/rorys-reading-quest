...
describe('getLessonForUnit', () => {
  ...
  test('exposes the active trail and bridge lessons while excluding legacy lessons', () => {
    ...
    expect(candidates).toHaveLength(273)
    expect(new Set(candidates.map((candidate) => candidate.activityId)).size).toBe(273)
    expect(candidates.filter((candidate) => candidate.gradeBand === 2)).toHaveLength(154)
    expect(candidates.filter((candidate) => candidate.gradeBand === 3)).toHaveLength(119)
