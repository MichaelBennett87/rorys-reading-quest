...
describe('multi-grade curriculum track architecture', () => {
  ...
  test('defines six immutable Grade 3 tracks with separate skills and Grade 2 prerequisites', () => {
    ...
    expect(grade3Tracks.filter((track) => !['g3-word-forge-foundations', 'g3-story-scouts-prose', 'g3-poetry-planet', 'g3-information-detectives-reading'].includes(track.trackId)).every((track) => track.status === 'planned_until_content_exists')).toBe(true)
