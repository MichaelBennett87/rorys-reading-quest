...
describe('Phase 7C3 acceptance documentation and boundary', () => {
  ...
  test('keeps R.2.3 implemented DRAFT, R.2.4 planned, and current totals exact', () => {
    ...
    expect(snapshot.rows.find((row) => row.benchmarkReference === 'ELA.3.R.2.4')).toMatchObject({ coverageStatus: 'planned', reviewStatus: 'DRAFT' })
    ...
    expect(getActiveContentRegistryTotals()).toEqual({ activePackCount: 33, activeLessonCount: 231, activePassageCount: 238, activeQuestionCount: 1327, activeSupportTargetCount: 915 })
  })
})
