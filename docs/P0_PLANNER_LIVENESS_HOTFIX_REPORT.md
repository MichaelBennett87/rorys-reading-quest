...
## Phase 7D7 final preservation reconciliation

The final Grade 3 audit re-runs registry-wide liveness, complete-track, active-session, exact-once, review-after-completion, and persistence contracts without changing planner thresholds or ordering. Fresh compatible work remains preferred, safe recycling remains available, completed tracks do not relaunch, `CONTENT_NEEDED` remains genuine, and the persisted singular `reasonCode` property remains absent while legitimate `reasonCodes` data remains supported. Final Grade 3 completion uses the existing one-action outcome and does not fabricate Phase 8 curriculum.

## P0 unit-affine review-completion follow-up

Deployed Phase 7D7 browser acceptance exposed a separate review-completion defect after planning had correctly selected a historical Grade 2 Story Map review on a completed track. Launch purpose and exact queue identity were not retained by the active session, so completion entered the ordinary current-difficulty progression path. The historical review difficulty did not equal the completed track difficulty; the result was declined, the session was cleared, no attempt or reward was recorded, the queue entry remained unchanged, and a false content-needed outcome appeared.

The bounded repair preserves review purpose and exact affinity across launch, Save and Exit, reload, and completion. Recovery validates the registered lesson and exactly one queue entry before granting review authority. A dedicated review transition records exact-once history and rewards, reschedules only the matching review through the unchanged interval policy, and preserves current difficulty, last mastered difficulty, qualifying evidence, failure counters, remediation, unrelated reviews, chapter completion, schema version 1, and storage keys. Historical lessons without authoritative review context remain rejected by ordinary difficulty validation.

Focused coverage includes completed Grade 2 and Grade 3 historical reviews, an incomplete advanced track, same-difficulty reviews, independent, assisted, partial, and unsuccessful results, duplicate completion, late checkpoints, Save and Exit plus reload, same-skill multiple-unit queues, and forged skill, difficulty, unit, version, lesson, due-time, step, cross-grade, and missing-entry identities. The existing safe-recycling and Fluency Flight completion implementations are unchanged.
