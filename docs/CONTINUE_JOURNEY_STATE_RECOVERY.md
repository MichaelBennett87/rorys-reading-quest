...
## Current question-first launcher supersession

The original Start Journey and Continue Journey controls are no longer rendered on the ordinary child route. Their authoritative transition, `prepareJourneyLaunch`, remains the single source of launch truth and now runs idempotently during default-route boot and after a final-question `Next`. Valid unfinished sessions still resume; completed or incompatible identities are retired; stale content-needed is recomputed; review and remediation purpose persists; and at most one new session is created.

Removing the visible continuation screens does not remove their invariants. Submitted feedback survives reload, final completion records one attempt and one reward transaction, historical unit-affine reviews reschedule exactly once, and the next planner-selected activity opens without trusting a stale rendered outcome. Genuine exhausted curriculum renders one calm completion state with no automatic retry loop.