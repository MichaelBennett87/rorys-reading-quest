...
`planGlobalQuest` remains the sole child lesson-selection authority. Active-session recovery, stored urgent plans, due reviews, verification, and remediation retain their established priority. Ordinary progression selects the first incomplete playable track by canonical `curriculumOrder`, refuses to bypass that required track, and proceeds only after it reaches its existing `completionDifficulty`. Within any compatible skill, difficulty, purpose, unit, and content-version boundary, recent activity and passage-question history rank candidates instead of banning them: unused non-overlapping work ranks first, unused overlapping work second, least-recently-used non-immediate recycling third, and an immediate repeat is the final sole-candidate fallback. Completion results are reconciled through the same global planner so the single `Continue Journey` action launches the current authoritative next lesson, including cross-world transitions. Schema version 1, storage keys, curriculum packs, scoring, mastery evidence, review timing, rewards, and parent analytics are unchanged.
