...
## Question-first child journey architecture

`AppShell` branches first on the explicit `#/parent` hash route. The parent branch enters the existing PIN gate without preparing child work. The default branch idempotently invokes `prepareJourneyLaunch`, recovers a compatible unfinished session or creates at most one planner-authorized session, and renders `LessonScreen` or `FluencyPracticeScreen` immediately. A brief noninteractive loading state is the only boot intermediary. Home, world-map, lesson-ready, lesson-results, and progression-outcome components remain historical or reusable code, but they are not mandatory stops or child navigation destinations in the current shell.

`planGlobalQuest` remains the sole lesson-selection authority. Active-session recovery, stored urgent plans, due reviews, verification, and remediation retain their established priority. Ordinary progression selects the first incomplete playable track by canonical `curriculumOrder` and refuses to bypass it. Recent activity and passage-question history remain deterministic variety rankings rather than exclusions, preserving safe same-track recycling and sole-candidate repetition.

The question screen owns one action-state transition: `Check Answer` records one submitted checkpoint and exposes persistent feedback; `Next` advances one question or completes the stable session identity once. Final completion updates progression/reviews/rewards, clears only that session, replans from the new authoritative state, and renders the next activity without a child-facing results or continuation route. Action locking prevents a single interaction burst from submitting and advancing twice. Multiselect and Hot Text readiness uses response structure, not hidden correct-answer cardinality.

Active sessions preserve bounded ID-only draft state, submitted feedback, question position, assistance, and review-aware launch context in schema version 1. The three storage keys, curriculum packs, scoring, mastery evidence, review timing, rewards, Parent PIN, assessments, and parent analytics remain unchanged.
...
## Phase 7C4 claim-evidence architecture

`ContentPack.authorClaimGuides` is optional authored curriculum metadata. Each DRAFT guide binds one informational argument passage to an explicit supportable claim, at least two reasons, at least three typed evidence records, resolved learner-visible source IDs, weak or irrelevant details, and explicit reason-to-evidence and evidence-to-claim connections. `buildAuthorClaimGuideAudit` validates ownership, exact pack shape, claim/topic/central-idea/purpose/fact boundaries, reason/evidence separation, cross-section checkpoint evidence, kind diversity, non-circular support, review status, and version without adding a renderer or scored response type.

The production registry activates `g3-id-unit-4` at difficulty 4. Verification, remediation, and review retain exact unit and content-version affinity. The Phase 7C3 `prepareJourneyLaunch` transition remains the sole Start/Continue authority, so stale content-needed is recomputed and no child-facing selection or conflict state is introduced. Authored guides remain outside schema-version-1 persistence and print output.
## Phase 7D1 figurative-language architecture

`ContentPack.figurativeLanguageGuides` is optional, authored curriculum metadata. Each Figurative Fortress text owns one guide with four discriminated metaphor, personification, or hyperbole targets. Targets bind exact learner-visible expressions and context evidence to a literal reading, nonliteral meaning, and device-specific fields. The validator rejects unresolved evidence, source-format mismatch, simile-shaped metaphors, normal animal behavior classified as personification, unclear hyperbole, dual classification, factual conflict, HTML, remote URLs, version mismatch, and non-DRAFT guide state.

The pack reuses `LessonTextCard`, `PoemCard`, `InformationalTextCard`, Word Help, source-evidence resolution, the hardened five evaluators, semantic audit, and per-pack truth ledgers. Guide data is neither persisted nor printed. `g3-across-genres-reading` is active only because Unit 1 production content exists; planned Grade 3 Context Cavern does not block the next production-ready track.
## Phase 7D2 summary architecture

`ContentPack.summaryGuides` is optional non-persisted curriculum metadata with a literary/informational discriminated union. The audit resolves source ownership, important and minor evidence, literary plot/theme fields, informational central-idea fields, summary compression, DRAFT status, and content version. Summary Stronghold reuses the existing prose and informational renderers, question evaluators, truth-ledger pipeline, planner, review affinity, parent reporting, and print boundary. No launcher, persistence schema, storage key, or scoring framework changed.

## P0 planner-liveness architecture

`selectNextLessonWithDiagnostics` separates hard compatibility from soft variety ranking and returns in-memory selection metadata for deterministic tests. The ordinary `selectNextLesson` API strips that metadata and returns only the existing `NextQuestPlan`, so selection mode, use counts, timestamps, overlap counts, and reason codes do not enter local persistence, parent reporting, or child UI. Incompatible historical versions are ignored for ranking without deleting valid history. `CONTENT_NEEDED` now means that no compatible authored candidate exists or that required unit/content-version affinity cannot be met, never merely that every compatible activity was recently used.

Terminal Grade 3 Fluency Flight completion is intentionally separate from selector exhaustion. `completeFluencyPractice` receives existing completed attempts, joins their exact lesson/activity identity to the current authored Unit 4 skill, difficulty, unit, and content version, includes the current completion, and advances to completion difficulty 5 only when every current authored activity is represented. The join adds no persisted field or schema migration. Before that boundary, and for nonterminal Grade 2 fluency, the same diagnostic selector continues to prefer fresh work and safely recycle compatible practice.

This liveness repair is separate from the Phase 7C3 active-session repair. The earlier transition still rejects completed or incompatible sessions, ignores late checkpoints, preserves duplicate-completion idempotency, recomputes stale plans, and gives Start Journey and Continue Journey one authoritative launch path.

## Phase 7D3 author-comparison architecture
...
## Phase 7D6 Meaning Maze architecture

`ContentPack` supports optional `MeaningMazeGuide` metadata with exact target form, challenge kind, primary and secondary strategies, source and context evidence, context-clue kind, word relationship, local reference IDs, broad background knowledge, alternate senses, literal readings, and confirmation statements. The metadata is validated at registration and never enters progress persistence or print output.

Informational sources reuse glossary features and may expose an authored local `reference` feature. `ReferenceMaterialCard` renders dictionary-style and thesaurus-style senses with semantic labels and responsive list markup. It does not create a general dictionary subsystem or remote dependency. Structured issues reject missing targets, noncontiguous phrases, clue or relationship mismatches, missing references, unsupported background knowledge, ambiguous senses or figurative meanings, incomplete combined strategies, morphology drift, and unsupported meanings.

The production registry adds only `g3-context-cavern-meaning-maze`. Existing paired-text, poem, literary, informational, question evaluator, persistence, planner, reward, review, and assessment architectures remain unchanged.
...
- browser requests use a fixed service base and a high-entropy installation bearer held in a dedicated IndexedDB credential store. Server code resolves the source and rubric from its own catalog, rejects browser model/URL/rubric authority, reserves a conservative neuron estimate atomically before inference, and binds deduplication to installation, operation, request, and payload identity.
- the guarded runtime is one Cloudflare Worker using direct Workers AI bindings and one SQLite-backed Durable Object. The Durable Object serializes installation authorization, request reservation, payload-bound deduplication, and per-UTC-day application quota; inference happens outside its short transactions. AI Gateway, anonymous inference, paid overages, and paid-provider fallback are absent.
- recognition and educational evaluation remain separate. Gemma output is parsed and validated without assuming JSON Mode; Llama Guard safety results are checked before child-visible feedback. Observed token usage, estimated neurons, unknown outcomes, and actual paid spending are represented separately. Unknown outcomes retain their reservation and are never blindly retried.
- the older single-instance Node/OpenAI implementation remains only as historical controlled-contract test infrastructure. It is not built as the guarded production Worker and cannot be selected as a quota fallback.

This is the narrow exception to older historical statements that no backend or live AI existed. Ordinary reading remains local. The Cloudflare Worker is implemented and tested with a controlled provider, but no account was available for deployment or live inference, so external activation remains pending.
