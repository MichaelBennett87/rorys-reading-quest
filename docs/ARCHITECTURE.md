...
- browser requests use a fixed service base and a high-entropy installation bearer held in a dedicated IndexedDB credential store. Server code resolves the source and rubric from its own catalog, rejects browser model/URL/rubric authority, reserves a finite budget atomically before inference, and binds deduplication to operation, request, and payload identity.
- the OpenAI adapter is server-only. It has no tools, web access, persistent conversation, file, or vector-store state. Recognition and educational evaluation are separate calls with parsed moderation decisions, generated-output safety checks, explicit token bounds, and observed-usage cost reconciliation.
- the repository-owned runtime is a deliberately single-instance Node service using a short file lock and atomic rename on an encrypted attached volume. It retains sensitive deduplication results for 15 minutes and non-content budget/auth audit metadata for 30 days. Multi-instance hosting requires a transactional ledger replacement.
