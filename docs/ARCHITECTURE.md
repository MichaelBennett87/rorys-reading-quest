...
- The active Grade 2 bridge packs now total nineteen registered packs, 133 lessons, 133 passages, 766 questions, and 530 support targets, with the legacy development pack still preserved for recovery and history.
...
Compare Castle remains a planned sequential world shell until production across-genres content exists. The shared curriculum architecture now reserves `g2-across-genres-reading` for `compare-castle` with three planned units:

- `cg-unit-1` Wordplay Watchtower for `ELA.2.R.3.1`
- `cg-unit-2` Retell Hall for `ELA.2.R.3.2`
- `cg-unit-3` Compare Keep for `ELA.2.R.3.3`

Phase 6F0 adds no production lessons and no new persistence behavior. It only establishes the world, track, roadmap, benchmark inventory, coverage snapshot, and documentation boundaries needed for later across-genres phases.
