...
Phase 6A1 adds the first scalable Grade 2 content pack without changing the lesson engine or parent systems. Phase 6A2 adds the second registered Word Forge pack. Together the bridge packs are DRAFT-only, locally authored, and registered as one active Word Forge unit plus legacy history-preserving content. They cover the full listed `ELA.2.F.1.3a` pattern set through `oo`, `ea`, `ou`, `oi`, `oy`, and `ow`, and they introduce guided-teaching lessons that support practice without pretending to score oral fluency. The work is supportive exposure and implemented DRAFT benchmark coverage, not complete benchmark mastery.
