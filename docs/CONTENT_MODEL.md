...
The active bridge packs remain DRAFT-only. Support is authored, not generated, and passage rendering keeps the readable sentence flow while exposing target words as controls. Together the packs implement the listed `ELA.2.F.1.3a` vowel-team patterns across Phase 6A1 and Phase 6A2, implement `ELA.2.F.1.3b` in Phase 6B1, implement `ELA.2.F.1.3c` across Phase 6B1 and Phase 6B2, and provide partial `ELA.2.F.1.3d` coverage across Phase 6C1 and Phase 6C2. The work stays local practice, not oral-fluency measurement.
