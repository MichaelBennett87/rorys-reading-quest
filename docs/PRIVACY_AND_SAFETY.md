...
## P0 learning-continuity storage boundary

The child-progress store remains schema version 1 under the existing key. It verifies successful writes by exact local read-back, refuses to overwrite unsupported or unreadable payloads, and rejects writes from a page whose loaded storage value is older than the current value. Recovery may discard only malformed transient session, plan, or outcome fields after all durable progress fields independently validate. No saved payload, diagnostic, passage, answer, PIN, or assessment is uploaded or copied to another store.
